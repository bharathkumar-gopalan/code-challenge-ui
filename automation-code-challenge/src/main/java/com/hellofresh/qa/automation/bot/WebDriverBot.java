...
import com.hellofresh.qa.automation.environment.ConfigurationHelper.Browser;
...
public class WebDriverBot implements Bot {
	...
	private static final String ELEMENT_PRESENCE_TIMEOUT_MESSAGE = "Timed out waiting for the element %s to be present";
	private static final String ELEMENT_PRESENCE_AND_VISIBILITY_TIMEOUT_MESSAGE = "Timed out waiting for the element %s to be present and visible";
	...
	private WebElement waitForElementToBePresent(PageElement element, int timeoutInSeconds) {
		...
		final WebElement webElement = wait.until(ExpectedConditions.presenceOfElementLocated(selector));
		return webElement;
	}
	
	private WebElement waitForElementToBeDisplayed(PageElement element, int timeoutInSeconds){
		final By selector = this.getByLocator(element);
		final FluentWait<WebDriver> wait = new FluentWait<WebDriver>(this.driver)
				.withTimeout(Duration.ofSeconds(timeoutInSeconds)).ignoring(RuntimeException.class)
				.withMessage(String.format(ELEMENT_PRESENCE_AND_VISIBILITY_TIMEOUT_MESSAGE, element))
				.pollingEvery(Duration.ofMillis(POLL_INTERVAL_IN_MILLIS));
		final WebElement webElement = wait.until(ExpectedConditions.visibilityOfElementLocated(selector));
		return webElement;
	}
	
	private WebElement waitForElementToBeDisplayed(final PageElement element) {
		return this.waitForElementToBeDisplayed(element, ELEMENT_TIMEOUT_IN_SECONDS);
	}

	private WebElement waitForElementToBePresent(final PageElement element) {
		return this.waitForElementToBePresent(element, ELEMENT_TIMEOUT_IN_SECONDS);
	}
	...
	public void click(PageElement element) {
		...
		final WebElement webElement = this.waitForElementToBeDisplayed(element);
		webElement.click();
		LOGGER.debug("Clicked on the element {}", element);
	}
	
	public String getPageUrl(){
		final String pageUrl = this.driver.getCurrentUrl();
		LOGGER.debug("The current page url is {}", pageUrl);
		return pageUrl;
	}
	...
	public String getText(PageElement element) {
		...
		final WebElement webElement = this.waitForElementToBePresent(element);
	...
	public boolean isElementPresent(PageElement element, int timeoutInSeconds) {
		...
		try {
			this.waitForElementToBePresent(element, timeoutInSeconds);
	...
	public void clickOnElementContainingText(PageElement element, String elementText) {
		LOGGER.debug("Clicking on the element {} containing text {}", element, elementText);
		...
		final FluentWait<WebDriver> wait = new FluentWait<WebDriver>(this.driver)
				...
				.withMessage(String.format(ELEMENT_PRESENCE_TIMEOUT_MESSAGE, element))
				.pollingEvery(Duration.ofMillis(POLL_INTERVAL_IN_MILLIS));
		final List<WebElement> webElements = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(locator));
		for (WebElement webElement : webElements) {
			final String webElementText = webElement.getText().trim();
			LOGGER.debug("Located the text {}", webElementText);
			if (webElementText.contains(elementText)) {
	...
	public void selectDropDownByValue(PageElement element, String value) {
		LOGGER.debug("Selecting the dropdown {} with value {}", element , value);
		final WebElement webElement = this.waitForElementToBePresent(element);
	...
	public void selectDropDownByVisibleText(PageElement element, String visibleText) {
		LOGGER.debug("Selecting the dropdown {} with visible text {}", element , visibleText);
		final WebElement webElement = this.waitForElementToBePresent(element);
		Select select = new Select(webElement);
	...
	public boolean isElementDisplayed(final PageElement element) {
		LOGGER.debug("Checking if the element {} is present and visible", element);
