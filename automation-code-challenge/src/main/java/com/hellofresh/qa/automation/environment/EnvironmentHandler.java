package com.hellofresh.qa.automation.environment;
...
public enum EnvironmentHandler {
	...
	private static final String GENERIC_FAILURE_MESSAGE = "An error has occurred while reading the property file , The message was %s";
	private static final String WEB_URL_NOT_DEFINED_MESSAGE = "There is no web url defined in the web-url property ";
	...
	private Properties readPropsForEnvironment(final Environment environment) {
		...
		try {
			...
			throw new RuntimeException(String.format(GENERIC_FAILURE_MESSAGE, e.getMessage()));
		}
	...
	public String getWebUrlFor(Environment environment) {
		...
		if (url == null || url.isEmpty()) {
			throw new RuntimeException(WEB_URL_NOT_DEFINED_MESSAGE);
		}
